...
class Checkout extends Component
{
    ...
    public function handlePaymentSuccess(string $intentId, AirwallexService $service): void
    {
        ...
        try {
            ...
            $attempt = $intent['latest_payment_attempt'] ?? null;
            $pm = $attempt['payment_method'] ?? null;
            $card = $pm['card'] ?? null;

            if ($pm && isset($pm['id']) && $card) {

                $alreadyExists = \App\Models\PaymentProfileLog::where('userId', $user->id)
                    ->where('payment_method_id', $pm['id'])
                    ->exists();

                if (!$alreadyExists) {

                    \App\Models\PaymentProfileLog::create([
                        'userId' => $user->id,
                        'orderId' => $orderId,
                        'order_total' => $paidAmount,
                        'creationdate' => now(),
                        'payment_method_id' => $pm['id'],
                        'consent_id' => $attempt['payment_consent_id'] ?? null,
    ...
    public function payWithSavedCard(AirwallexService $service)
    {
        $user = Auth::user();

        if (!$this->selectedCardMethodId) {
            return;
        }

        $intent = $service->createPaymentIntent(
            amount: $this->grandTotal,
            orderId: (string) $this->currentOrderId,
            customerId: $user->airwallex_customer_id
        );

        $service->confirmPaymentIntentWithSavedCard(
            intentId: $intent['id'],
            customerId: $user->airwallex_customer_id,
            paymentMethodId: $this->selectedCardMethodId,
