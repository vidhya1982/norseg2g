...
class Checkout extends Component
{
    ...
    public string $paymentMethod = 'card'; // ✅ JS se @this.set() se update hoga
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  Step 1: Pay Now — DB record + Airwallex PaymentIntent create karo
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public function payNow(AirwallexService $service): void
    {
        ...
        try {
            ...
            // ✅ method bhi dispatch karo taaki JS mein sahi element show ho
            $this->dispatch(
                'startPayment',
    ...
    public function handlePaymentSuccess(string $intentId, AirwallexService $service): void
    {
        ...
        try {
            ...
            if (!$orderId) {
                throw new \Exception('Session order ID missing');
            }

            $order = OrdersInitiated::findOrFail($orderId);

            // Idempotency — pehle se paid hai toh skip
            ...
            // ✅ Server-side verify — JS pe kabhi trust mat karo
            ...
            // eSIM Activation Jobs (uncomment when ready)
